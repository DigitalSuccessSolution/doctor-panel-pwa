    ...
    <div className="space-y-6 animate-fadeIn pb-16 font-sans">
        ...
        <div>
          <div className="flex items-center gap-2.5">
            ...
            <div>
              ...
              <span className="inline-flex items-center gap-1 bg-sky-50 text-sky-700 text-[10px] font-bold px-2 py-0.5 rounded-lg border border-sky-200 whitespace-nowrap mt-1">
                Select a patient to manage prescriptions
              </span>
            </div>
          </div>
          <p className="text-xs text-slate-500 font-medium mt-2">
            Browse patients to create, edit, or view their digital pediatric e-prescriptions.
          </p>
        </div>
      </div>
      ...
      <div className="relative">
        ...
        <input
          ...
          className="w-full bg-white border border-slate-200/80 rounded-2xl pl-11 pr-4 py-3 text-xs sm:text-sm text-slate-800 placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-sky-200 shadow-2xs"
        />
      </div>
              ...
              {filteredPatients.map((child) => (
                <tr key={child.id} className="hover:bg-slate-50/50 transition-colors text-xs">
                  ...
                  <td className="p-4 text-right">
                    <Link
                      href={`/patients/${child.id}?tab=prescriptions`}
                      className="inline-flex items-center gap-1 text-xs font-bold text-sky-700 bg-sky-50 hover:bg-sky-100 border border-sky-200 px-4 py-2.5 rounded-xl transition-all hover:scale-[1.02] active:scale-95 cursor-pointer"
                    >
          ...
          {filteredPatients.map((child) => (
            <div key={child.id} className="p-4 space-y-3.5 hover:bg-slate-50/50 transition-colors">
              ...
              <div className="flex items-center justify-between gap-3 pt-1">
                ...
                <Link
                  href={`/patients/${child.id}?tab=prescriptions`}
                  className="bg-[#1E4E70] hover:bg-[#153852] text-white font-bold text-xs px-4 py-2.5 rounded-xl flex items-center gap-1.5 shadow-2xs active:scale-95 transition-all"
                >
                  <FileText className="w-3.5 h-3.5" />
