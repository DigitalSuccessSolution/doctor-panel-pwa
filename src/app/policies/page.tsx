        ...
        <div className="flex items-center justify-between gap-4 pb-4 border-b border-slate-100">
          <div className="flex items-center gap-3.5 min-w-0">
            ...
            <div className="min-w-0">
              <h1 className="text-base sm:text-2xl font-semibold text-slate-900 tracking-tight leading-snug truncate">
                Policies & Governance
              </h1>
          ...
          {policyItems.map((item) => {
            ...
            return (
              <Link
                key={item.href}
                href={item.href}
                className="p-5 border border-slate-200/80 hover:border-[#1E4E70] bg-white hover:bg-slate-50/80 rounded-2xl flex items-center justify-between gap-4 group cursor-pointer transition-all shadow-xs"
              >
                <div className="flex items-start gap-3.5 min-w-0">
                  <div
                    className={`w-10 h-10 rounded-2xl flex items-center justify-center shrink-0 font-semibold mt-0.5 ${item.color.split(" ")[0]} ${item.color.split(" ")[1]}`}
                  >
